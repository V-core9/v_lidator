...
register = async (data) => {
  ...
  if (err.length === 0) {
    return (await v_database.item.new('users', await user_template(data), data.username) && await v_database.item.new('user_emails', await email_template(data), data.email));
  }
